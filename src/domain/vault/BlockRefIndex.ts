...
export function makeBlockRefIndex(
  ...
  for (const [page, list] of raw.entries()) {
    ...
    if (dups.length > 0) duplicatesByPage.set(normalize(page), dups);
  }

  const normalizedUnique = new Map<string, ReadonlySet<string>>();
  for (const [page, set] of unique.entries()) {
    normalizedUnique.set(normalize(page), set);
  }
