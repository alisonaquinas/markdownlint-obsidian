...
| 3 | `feature/phase-03-frontmatter-tags` | 2026-04-11 21:00 UTC | 2026-04-11 21:47 UTC | 22 | `e888322` (PR #2 via GitHub `--merge`) | ✅ run 24292196310 | Phase-2 review fixes #1 (typed `runRule` parameter) and #2 (freeze `ParseResult.frontmatter`) folded into Task 2's `RuleParams` refactor. Plan amendments: (a) OFM085 (duplicate-frontmatter-key) is unreachable in practice because gray-matter / js-yaml already throws OFM902 on duplicates — kept as a forward-compat net with synthetic-input unit tests; (b) OFM062 (empty-tag) joined OFM066/OFM082 as disabled-by-default because the bare-`#` detection over-fires on prose markdown; (c) `frontmatter.feature` and `tags.feature` tagged `@smoke`. `isIsoDate` now also accepts `Date` instances since gray-matter coerces unquoted YAML dates. Hook fallback (heredoc) used for `tests/integration/helpers/spawnCli.ts` (uses `node:child_process.spawn`) and the initial `OFM085` write (`regex.exec`; refactored to `string.match`). Reviewer approved with 4 non-blocking suggestions re: frontmatter line-number resolution for Phase 9 autofix, and a nit in `tags-integration.test.ts:46` missing `expect(r.exitCode).toBe(0)` — deferred. |
