...
| 6 | `feature/phase-06-block-refs-highlights` | 2026-04-12 00:10 UTC | 2026-04-12 00:32 UTC | 17 + 2 review fixes + 1 refactor | `4636fb5` (PR #5) | ✅ per PR | 10 rules (OFM100-104 + OFM120-124). `BlockRefIndex` domain service + builder. `VaultBootstrap` returns `{vault, blockRefs}`. OFM007 retired to alias. `FrontmatterParser cache: false` fix. Review found OFM120 glob/absolute-path bug + missing cache regression test — both fixed. |
| 7 | `feature/phase-07-standard-md` | 2026-04-12 02:12 UTC | 2026-04-12 02:54 UTC | 15 | `dd8c624` (PR #6) | ✅ per PR | Prior run stalled after creating the empty branch; re-dispatched. Integrated upstream `markdownlint` 0.40 via `markdownlint/sync` subpath. 43 MD rule descriptors registered; 6 conflicts disabled by default (MD013/018/033/034/041/042). Plan amended for real 0.40 API. `ConfigLoader` deep-merges `rules` block (latent bug: user config was wiping Phase 7 defaults). `test:dogfood` script now `cd docs &&` so per-dir `.obsidian-linter.jsonc` is discovered. 10 prose fixes (MD022/MD038) + 3 MD rules disabled in docs config (MD031/032/040) with rationale. File-steps BDD helper auto-appends `\n` for MD047. Reviewer approved; three non-blocking Phase 9 suggestions (fixInfo regression test, extractMdConfig per-rule retranslation, ConfigLoader option-map nesting). |
