...
| 5 | `feature/phase-05-embeds-callouts` | 2026-04-11 22:40 UTC | 2026-04-11 23:22 UTC | 19 | `25369b9` (PR #4) | ✅ per PR | 11 rules (OFM020-025 + OFM040-044). `OFMRule.run` contract widened to `void \| Promise<void>` for OFM022's async `fsCheck.exists`. Callout allowList expanded to 16 Obsidian-vanilla types. OFM021/OFM041 embed their own fence-skip logic because they scan raw lines (tech debt — reviewer recommends exposing `CodeRegionMap` through `ParseResult` in a future phase). Two rules renamed from plan file-map: `OFM025-embed-size-on-non-image.ts` (was `-extension-mismatch`) and `OFM044-callout-fold-disabled.ts` (was `-fold-on-note`). Integration test timeouts bumped to 20s for Windows parallel-spawn stability. `docs/bdd/steps/file-steps.ts` at 205 lines (warning, non-blocking). |
