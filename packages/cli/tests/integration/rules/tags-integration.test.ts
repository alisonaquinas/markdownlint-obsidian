...
const FIXTURES = fileURLToPath(
  new URL("../../../../core/tests/fixtures/rules/tags", import.meta.url),
);
...
describe("tag rules integration", () => {
  it("fails with OFM060 on a malformed tag", async () => {
    await fs.copyFile(path.join(FIXTURES, "bad-format.md"), path.join(tmp, "note.md"));
  ...
  it("reports OFM064 (warning) but exits 0 for case-insensitive duplicates", async () => {
    await fs.copyFile(path.join(FIXTURES, "duplicate.md"), path.join(tmp, "note.md"));
    const r = await spawnCli(["**/*.md"], tmp);
